...
only on purpose, on a branch, with review. The freeze exists to stop *accidental* and
*drift* changes, not to forbid *deliberate* ones. **"Frozen between deliberate
...
| **You, the AI, during a CONTENT session** | **No — guardrailed** | Do NOT edit foundation files while doing content work, even if it seems helpful. This prevents drift and silent regressions. If a content task seems to *require* a foundation change, STOP and say so — do not proceed. |
...
*without explicit instruction*.** Explicit instruction from the lead dev *is* the
...
and the copy *inside* them. That's it. Content lives in data, never in the chassis.
